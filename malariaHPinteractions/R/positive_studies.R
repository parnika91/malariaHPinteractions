...
# 
# # 22148 rows
# ungrouped_experiments <- rbind.data.frame(rs_ungrouped_abstract_anopheles, rs_ungrouped_abstract_malaria, rs_ungrouped_abstract_mosquito, rs_ungrouped_abstract_plasmodium, rs_ungrouped_exptitle_anopheles, rs_ungrouped_exptitle_malaria, rs_ungrouped_exptitle_mosquito, rs_ungrouped_exptitle_plasmodium, rs_ungrouped_title_anopheles, rs_ungrouped_title_malaria, rs_ungrouped_title_mosquito, rs_ungrouped_title_plasmodium, rs_ungrouped_sample_anopheles, rs_ungrouped_sample_malaria, rs_ungrouped_sample_mosquito, rs_ungrouped_sample_plasmodium)
...
# rs_murine <- getSRA( search_terms = "murine", out_types = c('sra'), sra_con )
...
# rs_mouse <- getSRA( search_terms = "mouse", out_types = c('sra'), sra_con )
# rs_rodent <- getSRA( search_terms = "rodent", out_types = c('sra'), sra_con)
# rs_human <- getSRA( search_terms = "human", out_types = c('sra'), sra_con)
# 
# ungrouped_rs <- rbind.data.frame(rs_malaria, rs_plasmodium, rs_murine, rs_mouse, rs_rodent, rs_human) #1812703 rows
...
# unique_ungrouped_rs_runs <- dbGetQuery(db, paste("SELECT *, count(*) AS Runs FROM `unique_ungrouped_rs.sqlite` WHERE library_strategy IS 'RNA-Seq' GROUP BY study_alias", sep=""))
...
# 
# write.table(check_unique_ungrouped_rs_runs, "check_unique_ungrouped_rs_runs.txt", sep = "\t", row.names = FALSE)
# 
# # 3886 - 134 rows
...
# This next step is to  retrieve the run IDs from each study ID. It needs positive_studies.txt as an input.
...
positive_studies <- read.table("positive_studies.txt", header = TRUE, sep = c("\t"))
...
for(i in index)
{
  rs = getSRAinfo(positive_experiments[index], sra_con, sraType = "sra")
  runs_rs <- c(runs_rs, rs[,5])
}
...
save(runs_rs, file = "runs_rs.RData")
write.table(runs_rs, "runs_rs.txt", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)