...
bash --verbose $default_path/Scripts/DownloadSRAtoolkit.sh $run # download run or pair of runs, based on layout=Paired or Single
...
if [ -e $run\_1.fastq ]; then
  # put .gz
  STAR --runThreadN 4 --genomeDir /SAN/Plasmo_compare/Genomes/indices/mousePchabaudi --readFilesIn /SAN/Plasmo_compare/fastq_download_tmp/$run\_1.fastq /SAN/Plasmo_compare/fastq_download_tmp/$run\_2.fastq --outSAMtype BAM Unsorted --outFileNamePrefix /SAN/Plasmo_compare/fastq_download_tmp/$run #--readFilesCommand zcat

else
  STAR --runThreadN 4 --genomeDir /SAN/Plasmo_compare/Genomes/indices/mousePchabaudi --readFilesIn /SAN/Plasmo_compare/fastq_download_tmp/$run.fastq --outFileNamePrefix /SAN/Plasmo_compare/fastq_download_tmp/$run --outSAMtype BAM Unsorted #--readFilesCommand zcat
